...
final class MethodChannelWorkspaceBridge implements WorkspacePlatformBridge {
  ...
  Future<WorkspaceOutcome<WorkspaceSelection>> selectDirectory(
    ...
    try {
      final envelope = await _channel.invokeMethod<Uint8List>('selectDirectory',
  ...
  Future<WorkspaceOutcome<WorkspaceDirectory>> restore(
    ...
    final response = await _call(
        'restore',
        _protocolArguments(
            {'workspaceId': id.value, 'envelope': nativeEnvelope}));
  ...
  Future<WorkspaceOutcome<WorkspaceRead>> read(
    ...
    try {
      ...
      final stability = _validatedStability(
          reportedStability, actualRevision, request.expectedRevision);
  ...
  Future<WorkspaceOutcome<Map<Object?, Object?>>> _operationCall(String method,
      OperationBudget budget, Map<String, Object?> arguments) async {
    if (budget.cancellationToken.isCancelled) {
      return const WorkspaceFailure(WorkspaceFailureKind.cancelled);
    }
    final remainingMillis =
        budget.deadline.difference(DateTime.now()).inMilliseconds;
    ...
    try {
      final outcome = await _call(
          ...
          _protocolArguments(<String, Object?>{
            ...
            'remainingMillis': remainingMillis.clamp(1, 86400000) as int,
          }));
  ...
  String _newOperationId() {
    ...
    return List<String>.generate(
        22, (_) => alphabet[random.nextInt(alphabet.length)]).join();
  }
    ...
    try {
      return WorkspaceSuccess(WorkspaceDirectory(
          ref: WorkspaceEntryRef.issued(
              workspaceId: id, stableId: map['entryId']! as String),
  ...
  WorkspaceEntry _entry(WorkspaceId id, Map<Object?, Object?> map) {
    ...
    return switch (map['type']) {
      'directory' when !map.containsKey('byteLength') =>
