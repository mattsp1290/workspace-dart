...
void main() {
  ...
  test('picker dismissal is a typed cancellation', () async {
    ...
    expect(
      (outcome as WorkspaceFailure<WorkspaceSelection>).kind,
  ...
  test('list registers per-operation cancellation before settlement', () async {
    ...
    expect(
      (outcome as WorkspaceFailure<WorkspacePage>).kind,
      WorkspaceFailureKind.cancelled,
    );
  });
  ...
  test('already-cancelled operations never dispatch', () async {
    ...
    expect(
      (outcome as WorkspaceFailure<WorkspaceRead>).kind,
      WorkspaceFailureKind.cancelled,
    );
  });

  test(
    'native verified claim without revision evidence is downgraded',
    () async {
      ...
      expect(
        (outcome as WorkspaceSuccess<WorkspaceRead>).value.stability,
  ...
  test(
    'all documented native failure codes retain their typed meaning',
    () async {
      ...
      for (final entry in expected.entries) {
        ...
        final outcome = await MethodChannelWorkspaceBridge(
          channel: channel,
